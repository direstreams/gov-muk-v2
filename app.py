...
def init_db():
    ...
    if db.execute("SELECT COUNT(*) FROM travel_notices").fetchone()[0] == 0:
        ...
        notices = [
            ...
            ("United States", "green", "Stable security", "Normal travel precautions apply. Security conditions are currently stable.", now),
            ("Germany", "green", "Stable security", "Normal travel precautions apply. Security conditions are currently stable.", now),
    ...
    db.execute(
        "UPDATE travel_notices SET level='green', headline='Stable security', details='Normal travel precautions apply. Security conditions are currently stable.', updated_at=? WHERE country IN ('United States','Germany')",
    ...
    db.execute(
        "UPDATE diplomatic_relations SET relationship=?, notes=?, updated_at=? WHERE country='United States'",
    ...
    db.execute(
        "UPDATE diplomatic_relations SET relationship=?, notes=?, updated_at=? WHERE country='Germany'",
    ...
    db.execute(
        "UPDATE emergency_alerts SET area='Ukraine', guidance='Exercise increased caution and review the current travel notice before departure.', updated_at=? WHERE title='Heightened international caution'",
        (datetime.utcnow().strftime("%Y-%m-%d %H:%M"),),
    )
    db.commit()
    db.close()
...
def emergency():
    ...
    notices = db.execute("SELECT * FROM travel_notices ORDER BY CASE level WHEN 'red' THEN 1 WHEN 'yellow' THEN 2 ELSE 3 END,country").fetchall()
...
def admin_recognition_save():
    ...
    vals=(request.form["country"],request.form["status"],request.form["relationship"],request.form.get("capital",""),float(request.form.get("latitude",0) or 0),float(request.form.get("longitude",0) or 0),request.form.get("notes",""),datetime.utcnow().strftime("%Y-%m-%d %H:%M"))
    ...
    db.commit(); flash("Diplomatic recognition entry saved.","success"); return redirect(url_for("admin_dashboard"))
...
def admin_operation_save():
    db=get_db(); item_id=request.form.get("id")
    vals=(request.form["name"],request.form["region"],request.form["status"],request.form["department"],request.form["summary"],float(request.form.get("latitude",0) or 0),float(request.form.get("longitude",0) or 0),datetime.utcnow().strftime("%Y-%m-%d %H:%M"))
