...
def init_db():
    ...
    db.executescript(
        ...
        CREATE TABLE IF NOT EXISTS service_status (
            ...
            status TEXT NOT NULL CHECK(status IN ('operational','degraded','maintenance','outage')),
        ...
        CREATE TABLE IF NOT EXISTS digital_services (
            ...
            url TEXT NOT NULL,
            status TEXT NOT NULL DEFAULT 'available' CHECK(status IN ('available','limited','offline'))
        );
    ...
    if db.execute("SELECT COUNT(*) FROM petitions").fetchone()[0] == 0:
        ...
        db.execute(
            ...
            ("Deport Fudgy", "This petition calls on the Government of MUK to consider the deportation of Fudgy under applicable fictional roleplay procedures.", "Government Citizen", "open", now),
        )
    ...
    if db.execute("SELECT COUNT(*) FROM service_status").fetchone()[0] == 0:
        now = datetime.utcnow().strftime("%Y-%m-%d %H:%M")
        db.executemany(
            "INSERT INTO service_status(service_name,status,message,updated_at) VALUES(?,?,?,?)",
            [
                ("GOV.MUK website", "operational", "All public pages are available.", now),
                ("Bank of England", "operational", "Citizen banking services are available.", now),
                ("Travel advice", "operational", "Country advisories are being published normally.", now),
                ("Emergency centre", "operational", "National alerts are available.", now),
                ("Petitions", "operational", "Petition viewing and signing are available.", now),
                ("Diplomatic map", "operational", "Map and recognition register are available.", now),
            ],
        )

    if db.execute("SELECT COUNT(*) FROM digital_services").fetchone()[0] == 0:
        db.executemany(
            "INSERT INTO digital_services(title,description,department,url,status) VALUES(?,?,?,?,?)",
            [
                ("View foreign travel advice", "Check current fictional travel warnings by country.", "Foreign Affairs", "/travel", "available"),
                ("Sign a petition", "View and support open public petitions.", "Cabinet Office", "/petitions", "available"),
                ("Apply to 42 Commando", "Open the Royal Marines recruitment community.", "Ministry of Defence", "https://discord.gg/7FAbWyMDfX", "available"),
                ("Apply to 856 Support Unit", "Open the Royal Marines recruitment community.", "Ministry of Defence", "https://discord.gg/7FAbWyMDfX", "available"),
                ("View active warrants", "Open the public fictional warrant register.", "Minister of State", "/warrants", "available"),
                ("Check service status", "See the live operational status of GOV.MUK services.", "Cabinet Office", "/status", "available"),
                ("View diplomatic relations", "Explore recognition and international relations.", "Foreign Affairs", "/diplomacy", "available"),
                ("Open Bank of England", "Access administrator-issued citizen banking services.", "Treasury", "/bank", "available"),
            ],
        )

    if db.execute("SELECT COUNT(*) FROM news_items").fetchone()[0] == 0:
        db.executemany(
            "INSERT INTO news_items(title,summary,department,image_url,published_at,is_featured) VALUES(?,?,?,?,?,?)",
            [
                (
                    "MOD withdraws from Northern Ireland war against Ireland",
                    "The Ministry of Defence confirmed the withdrawal following an operational review.",
                    "Ministry of Defence",
                    "/static/images/mod-withdrawal.webp",
                    "1 August 2026",
                    1,
                ),
                (
                    "Government expands public digital services",
                    "New travel, petitions, diplomatic and status services are now available.",
                    "Cabinet Office",
                    "",
                    "2 August 2026",
                    0,
...
def home():
    ...
    latest_news = db.execute("SELECT * FROM news_items ORDER BY is_featured DESC, id DESC LIMIT 4").fetchall()
    notices = db.execute(
        "SELECT * FROM travel_notices ORDER BY CASE level WHEN 'black' THEN 1 WHEN 'red' THEN 2 WHEN 'amber' THEN 3 WHEN 'yellow' THEN 4 WHEN 'green' THEN 5 ELSE 6 END, country LIMIT 6"
    ).fetchall()
    services = db.execute("SELECT * FROM digital_services ORDER BY id LIMIT 8").fetchall()
    statuses = db.execute("SELECT * FROM service_status ORDER BY id LIMIT 6").fetchall()
    petitions = db.execute("SELECT * FROM petitions ORDER BY id DESC LIMIT 3").fetchall()
    return render_template(
        "index.html",
        leaders=leaders,
        latest_news=latest_news,
        travel_notices=notices,
...
def search():
    ...
    def add_result(title, description, target, result_type, searchable):
        haystack = f"{title} {description} {searchable}".lower()
        if (not terms or all(term in haystack for term in terms)) and (category == "all" or category == result_type):
            results.append({
                "title": title,
    ...
    static_pages = [
        ("Services and information", "Browse all government services.", url_for("services"), "service", "services departments"),
        ("Foreign travel advice", "View security and conflict notices by country.", url_for("travel"), "travel", "travel country warnings"),
        ("Government and leadership", "View ministers and senior public offices.", url_for("leadership"), "government", "leaders cabinet ministers"),
        ("Government documents", "Search official publications and documents.", url_for("documents"), "document", "documents publications"),
        ("Ministry of Defence", "Recruitment, statements and defence information.", url_for("mod"), "government", "defence 42 commando 856 support"),
        ("Diplomatic recognition", "Explore diplomatic relationships and the world map.", url_for("diplomacy"), "diplomacy", "countries recognition relations"),
        ("Emergency centre", "View national warnings and service alerts.", url_for("emergency"), "emergency", "emergency warnings alerts"),
        ("Digital services", "Browse online GOV.MUK services.", url_for("digital_services"), "service", "online digital apply check"),
        ("Live status board", "Check whether GOV.MUK services are operational.", url_for("status_board"), "status", "systems status operational"),
    ]
    for item in static_pages:
        add_result(*item)

    for row in db.execute("SELECT * FROM travel_notices").fetchall():
        add_result(
            f"{row['country']} travel advice",
            f"{row['headline']} — {row['details']}",
            url_for("travel_country", item_id=row["id"]),
            "travel",
            f"{row['country']} {row['level']}",
        )

    for row in db.execute("SELECT * FROM documents").fetchall():
        add_result(row["title"], row["description"], row["url"], "document", row["category"])

    for row in db.execute("SELECT * FROM leaders").fetchall():
        add_result(row["name"], f"{row['office']} — {row['description']}", url_for("leadership"), "government", row["office"])

    for row in db.execute("SELECT * FROM diplomatic_recognition").fetchall():
        add_result(
            row["country"],
            row["relationship"],
            url_for("diplomacy"),
            "diplomacy",
            f"{row['capital']} {row['status']} {row['notes']}",
        )

    for row in db.execute("SELECT * FROM petitions").fetchall():
        add_result(row["title"], row["description"], url_for("petition_detail", petition_id=row["id"]), "petition", "petition signatures")

    for row in db.execute("SELECT * FROM digital_services").fetchall():
        add_result(row["title"], row["description"], row["url"], "service", row["department"])

    return render_template("search.html", query=query, category=category, results=results)
...
@app.route("/status")
def status_board():
    rows = get_db().execute("SELECT * FROM service_status ORDER BY id").fetchall()
    return render_template("status.html", statuses=rows)


@app.route("/digital-services")
def digital_services():
    rows = get_db().execute("SELECT * FROM digital_services ORDER BY department,title").fetchall()
    return render_template("digital_services.html", services=rows)


@app.route("/travel/<int:item_id>")
def travel_country(item_id):
    notice = get_db().execute("SELECT * FROM travel_notices WHERE id=?", (item_id,)).fetchone()
    if notice is None:
        return render_template("404.html"), 404
    return render_template("travel_country.html", notice=notice)
...
def admin_dashboard():
    ...
    return render_template("admin.html",
        ...
        stocks=db.execute("SELECT * FROM stocks ORDER BY symbol").fetchall(),
        service_statuses=db.execute("SELECT * FROM service_status ORDER BY id").fetchall(),
        digital_services=db.execute("SELECT * FROM digital_services ORDER BY department,title").fetchall(),
        recognition=db.execute("SELECT * FROM diplomatic_recognition ORDER BY country").fetchall(),
...
@app.post("/admin/status/save")
@admin_required
def admin_status_save():
    db = get_db()
    service_id = request.form.get("id")
    values = (
        request.form["service_name"].strip(),
        request.form["status"],
    ...
    if service_id:
        db.execute(
            "UPDATE service_status SET service_name=?,status=?,message=?,updated_at=? WHERE id=?",
            (*values, service_id),
        )
    else:
        db.execute(
...
@app.post("/admin/digital-service/save")
@admin_required
def admin_digital_service_save():
    db = get_db()
    service_id = request.form.get("id")
    values = (
    ...
    if service_id:
        db.execute(
            "UPDATE digital_services SET title=?,description=?,department=?,url=?,status=? WHERE id=?",
            (*values, service_id),
        )
    else:
        db.execute(
    ...
    db.commit()
    flash("Digital service saved.", "success")
    return redirect(url_for("admin_dashboard"))
