...
def _make_pdf_bytes(title, subtitle, fields, notice=None):
    lines = [(title, 18), (subtitle, 11), ("", 10)]
    for label, value in fields:
        wrapped = _pdf_wrap(value, 78)
        lines.append((f"{label}: {wrapped[0]}", 10))
        for extra in wrapped[1:]:
            lines.append((f"    {extra}", 10))
        lines.append(("", 5))
    if notice:
        for line in _pdf_wrap(notice, 88):
            lines.append((line, 9))
    lines.append(("", 6))
    lines.append(("FICTIONAL ROLEPLAY DOCUMENT - NOT VALID FOR REAL-WORLD USE", 8))

    page_chunks, chunk = [], []
    max_lines = 48
    for item in lines:
        if len(chunk) >= max_lines:
            page_chunks.append(chunk); chunk = []
        chunk.append(item)
    if chunk: page_chunks.append(chunk)
    ...
    for i in range(len(page_chunks)):
        page_obj_numbers.append(first_dynamic + i*2)
        content_obj_numbers.append(first_dynamic + i*2 + 1)
        kids.append(f'{page_obj_numbers[-1]} 0 R')
    objects.append(f'<< /Type /Pages /Kids [{" ".join(kids)}] /Count {len(page_chunks)} >>'.encode())
    ...
    for page_no, chunk in enumerate(page_chunks, start=1):
        ...
        page_obj = f'<< /Type /Page /Parent 2 0 R /MediaBox [0 0 595 842] /Resources << /Font << /F1 3 0 R >> >> /Contents {content_obj_numbers[page_no-1]} 0 R >>'.encode()
...
def _download_pdf(filename, title, subtitle, fields, notice=None):
    data = _make_pdf_bytes(title, subtitle, fields, notice)
    return send_file(
        BytesIO(data),
...
def init_db():
    ...
    db.executescript(
        ...
        CREATE TABLE IF NOT EXISTS petitions (
            ...
            government_response TEXT NOT NULL DEFAULT '',
        ...
        CREATE TABLE IF NOT EXISTS job_applications (
            ...
            unit TEXT NOT NULL CHECK(unit IN ('42 Commando','856 Support Unit')),
            ...
            criminal_record TEXT NOT NULL CHECK(criminal_record IN ('none','declared')),
            criminal_record_details TEXT NOT NULL DEFAULT '',
            availability TEXT NOT NULL,
            status TEXT NOT NULL CHECK(status IN ('submitted','reviewing','accepted','rejected')) DEFAULT 'submitted',
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL
        );
        ...
        CREATE TABLE IF NOT EXISTS driving_applications (
            ...
            issue_date TEXT,
            expiry_date TEXT,
            status TEXT NOT NULL CHECK(status IN ('started','passed','failed','suspended','revoked')) DEFAULT 'started',
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL
        );
        ...
        CREATE TABLE IF NOT EXISTS passport_applications (
            ...
            issue_date TEXT,
            expiry_date TEXT,
            status TEXT NOT NULL CHECK(status IN ('submitted','reviewing','approved','refused')) DEFAULT 'submitted',
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL
        );
        ...
        CREATE TABLE IF NOT EXISTS visa_applications (
            ...
            intended_arrival TEXT NOT NULL,
            intended_stay TEXT NOT NULL,
            accommodation TEXT NOT NULL,
            declarations TEXT NOT NULL,
            processing_time TEXT NOT NULL DEFAULT '10 working days',
            status TEXT NOT NULL CHECK(status IN ('submitted','reviewing','approved','refused')) DEFAULT 'submitted',
            created_at TEXT NOT NULL,
            updated_at TEXT NOT NULL
        );
    ...
    if db.execute("SELECT COUNT(*) FROM leaders").fetchone()[0] == 0:
        leaders = [
            ...
            ("Minister of Defence", "DIREVERCETTI", "Responsible for defence policy, armed forces and national security.", 2),
    ...
    if db.execute("SELECT COUNT(*) FROM petitions").fetchone()[0] == 0:
        ...
        db.execute(
            ...
            (
                "Deport Fudgy",
                "This petition calls on the Government of MUK to consider the deportation of Fudgy under applicable fictional roleplay procedures.",
                "Government Citizen",
                "open",
                now,
            ),
    ...
    # Add fields introduced in later V3 updates to existing SQLite databases.
    petition_columns = {row["name"] for row in db.execute("PRAGMA table_info(petitions)").fetchall()}
    if "category" not in petition_columns:
        db.execute("ALTER TABLE petitions ADD COLUMN category TEXT NOT NULL DEFAULT 'Other'")
    if "government_response" not in petition_columns:
        db.execute("ALTER TABLE petitions ADD COLUMN government_response TEXT NOT NULL DEFAULT ''")
    if "response_date" not in petition_columns:
        db.execute("ALTER TABLE petitions ADD COLUMN response_date TEXT NOT NULL DEFAULT ''")

    visa_columns = {row["name"] for row in db.execute("PRAGMA table_info(visa_applications)").fetchall()}
    if "processing_time" not in visa_columns:
        db.execute("ALTER TABLE visa_applications ADD COLUMN processing_time TEXT NOT NULL DEFAULT '10 working days'")
    ...
    # Keep diplomatic and operational map markers at useful, non-overlapping locations.
    corrected_recognition_coordinates = [
        ...
        ("Falkland Islands", "Stanley", -51.6977, -57.8517),
        ("Finland", "Helsinki", 60.1699, 24.9384),
        ("Lebanon", "Beirut", 33.8938, 35.5018),
        ("Dominican Republic", "Santo Domingo", 18.4861, -69.9312),
    ]
    for country, capital, latitude, longitude in corrected_recognition_coordinates:
        db.execute(
            "UPDATE diplomatic_recognition SET capital=?,latitude=?,longitude=?,updated_at=? WHERE lower(country)=lower(?)",
            (capital, latitude, longitude, datetime.utcnow().strftime("%Y-%m-%d %H:%M"), country),
        )

    corrected_operation_coordinates = [
        ("Operation Northern Watch", 54.7800, -6.1200),
        ("Operation Atlantic Guard", -52.0500, -58.6500),
        ("Operation Safe Passage", 51.8985, -8.4756),
    ]
    for operation_name, latitude, longitude in corrected_operation_coordinates:
        db.execute(
            "UPDATE operations SET latitude=?,longitude=?,updated_at=? WHERE name=?",
            (latitude, longitude, datetime.utcnow().strftime("%Y-%m-%d %H:%M"), operation_name),
        )

    agency_leaders = [
        ("Director of Defence Agencies (MOD)", "DIREVERCETTI", "Coordinates the public administration of defence agencies.", 5),
        ("Director of Foreign Affairs Agencies (FCDO)", "Hades", "Coordinates diplomatic and foreign-affairs agencies.", 6),
        ("NHS Chair", "nathan", "Chairs the fictional MUK National Health Service.", 7),
    ...
    # Update the default transport petition in existing databases.
    try:
        db.execute(
            """UPDATE petitions
               SET title=?, summary=?
               WHERE lower(title) LIKE '%tfl%'
                  OR lower(title) LIKE '%transport for london%'""",
            (
                "Deport Fudgy",
                "This petition calls on the Government of MUK to consider the deportation of Fudgy under applicable fictional roleplay procedures.",
            ),
...
@app.route("/government/community-management")
def community_management():
    oversight = ["direvercetti", "sirwilliamstarmer"]
    management = ["tri_st", "nha.yw", "petertoparse", "ruben07364", "thereal_skyz"]
...
def diplomacy():
    ...
    operations = db.execute(
        "SELECT * FROM operations ORDER BY CASE status WHEN 'active' THEN 1 WHEN 'monitoring' THEN 2 WHEN 'suspended' THEN 3 ELSE 4 END,name"
    ).fetchall()
    travel_rows = db.execute(
        "SELECT id,country,level,headline,details FROM travel_notices"
    ).fetchall()
    travel_by_country = {
    ...
    return render_template(
        ...
        travel_by_country=travel_by_country,
    )
...
def petitions():
    ...
    if request.method == "POST":
        ...
        allowed_categories = {
            "Defence", "Health", "Transport", "Justice", "Foreign Affairs",
            "Economy", "Public Services", "Other"
        }
        ...
        if len(title) < 5 or len(summary) < 20 or len(creator) < 2:
            ...
            db.execute(
                "INSERT INTO petitions(title,summary,creator,status,category,government_response,response_date,created_at) VALUES(?,?,?,?,?,?,?,?)",
                (
                    title,
                    summary,
                    creator,
    ...
    rows = db.execute(
        ...
        "FROM petitions p WHERE p.status IN ('open','closed')" + category_clause + " ORDER BY p.id DESC",
        params,
    ).fetchall()
    categories = [
        "Defence", "Health", "Transport", "Justice", "Foreign Affairs",
...
DRIVING_QUESTION_BANK = [{"question": "What should you do at a red traffic light?", "options": ["Stop behind the line", "Speed up", "Sound the horn", "Ignore it if the road is clear"], "answer": 0}, {"question": "What is the safest action before changing lane?", "options": ["Check mirrors and blind spot", "Only signal", "Brake sharply", "Move immediately"], "answer": 0}, {"question": "When should you use dipped headlights?", "options": ["At night or in poor visibility", "Only on motorways", "Only when parked", "Never"], "answer": 0}, {"question": "What should you do near a pedestrian crossing?", "options": ["Slow down and be ready to stop", "Accelerate", "Overtake other vehicles", "Use the horn continuously"], "answer": 0}, {"question": "What is the purpose of a seat belt?", "options": ["Reduce injury in a collision", "Improve fuel economy", "Increase speed", "Replace airbags"], "answer": 0}, {"question": "What should you do if an emergency vehicle approaches with lights and siren?", "options": ["Move aside safely when possible", "Race it", "Stop in the middle of the road", "Ignore it"], "answer": 0}, {"question": "Why should you keep a safe following distance?", "options": ["To allow time to stop", "To stop others overtaking", "To use less road", "To see the driver's face"], "answer": 0}, {"question": "What should you do before reversing?", "options": ["Check all around the vehicle", "Only look forward", "Sound the horn and reverse quickly", "Close your mirrors"], "answer": 0}, {"question": "When is it unsafe to overtake?", "options": ["Near a bend with limited visibility", "On a clear straight road", "When the road is empty", "After checking mirrors"], "answer": 0}, {"question": "What should you do if the road is wet?", "options": ["Increase following distance", "Drive closer to the car ahead", "Brake later", "Turn off headlights"], "answer": 0}, {"question": "What should you do when approaching a school area?", "options": ["Reduce speed and watch for children", "Accelerate", "Overtake buses", "Use full beam"], "answer": 0}, {"question": "What does a triangular road sign usually indicate?", "options": ["A warning", "A parking space", "A motorway exit", "A speed camera only"], "answer": 0}, {"question": "Why should you check tyre condition?", "options": ["For safe grip and braking", "To make the car louder", "To increase radio signal", "To avoid using mirrors"], "answer": 0}, {"question": "What should you do if you feel tired while driving?", "options": ["Stop somewhere safe and rest", "Open a window and continue indefinitely", "Drive faster", "Use your phone"], "answer": 0}, {"question": "What is the safest way to use a mobile phone while driving?", "options": ["Do not use it unless safely parked", "Hold it below the steering wheel", "Text at traffic lights", "Use it on quiet roads"], "answer": 0}]


@app.route("/driving-licence/apply", methods=["GET", "POST"])
def driving_licence_apply():
    if request.method == "POST":
        roblox_username = request.form.get("roblox_username", "").strip()
        discord_username = request.form.get("discord_username", "").strip()
        roleplay_name = request.form.get("roleplay_name", "").strip()
        ...
        if not roblox_username or len(roblox_username) > 50:
            errors.append("Enter a valid Roblox username.")
        if not discord_username or len(discord_username) > 80:
            errors.append("Enter a valid Discord username.")
        if not roleplay_name or len(roleplay_name) > 100:
            errors.append("Enter a fictional roleplay name.")
        if age_band not in {"13-15", "16-17", "18+"}:
            errors.append("Select an age band.")
        if licence_type not in {"provisional", "full"}:
            errors.append("Select a licence type.")
        if declaration != "yes":
            errors.append("Confirm this is fictional roleplay information.")

        if errors:
            for error in errors:
                flash(error, "error")
            return render_template("driving_apply.html")

        now = datetime.utcnow().strftime("%Y-%m-%d %H:%M")
        reference = f"DL-{datetime.utcnow().strftime('%Y%m%d')}-{secrets.token_hex(3).upper()}"
        db = get_db()
        db.execute(
            """INSERT INTO driving_applications(
                reference,roblox_username,discord_username,roleplay_name,
                age_band,licence_type,score,total_questions,passed,status,
        ...
        db.commit()
        session["driving_reference"] = reference
        session["driving_questions"] = random.sample(range(len(DRIVING_QUESTION_BANK)), 10)
        return redirect(url_for("driving_theory_test", reference=reference))

    return render_template("driving_apply.html")


@app.route("/driving-licence/test/<reference>", methods=["GET", "POST"])
def driving_theory_test(reference):
    application = get_db().execute(
        "SELECT * FROM driving_applications WHERE reference=?",
        (reference,),
    ).fetchone()
    if application is None:
        abort(404)

    question_ids = session.get("driving_questions")
    if not question_ids or len(question_ids) != 10:
        question_ids = random.sample(range(len(DRIVING_QUESTION_BANK)), 10)
        session["driving_questions"] = question_ids

    questions = [DRIVING_QUESTION_BANK[index] for index in question_ids]

    if request.method == "POST":
        score = 0
        for position, question in enumerate(questions):
            selected = request.form.get(f"q{position}")
        ...
        if passed:
            licence_number = f"MUK-{secrets.token_hex(4).upper()}"
            issue_date = now.strftime("%d %B %Y")
            expiry_date = now.replace(year=now.year + 5).strftime("%d %B %Y")
            status = "passed"

        db = get_db()
        db.execute(
            """UPDATE driving_applications
               SET score=?,passed=?,licence_number=?,issue_date=?,expiry_date=?,
                   status=?,updated_at=?
               WHERE reference=?""",
            (
                score,
        ...
        if passed:
            return redirect(url_for("driving_licence_complete", reference=reference))
        return render_template(
            "driving_result.html",
...
@app.route("/driving-licence/result/<reference>")
def driving_licence_complete(reference):
    application = get_db().execute(
        "SELECT * FROM driving_applications WHERE reference=?",
        (reference,),
    ).fetchone()
    if application is None:
        abort(404)
    return render_template("driving_result.html", application=application, passed=bool(application["passed"]))


@app.route("/passport-office")
def passport_office():
    return render_template("passport_office.html")


@app.route("/passport-office/apply/<application_type>", methods=["GET", "POST"])
def passport_apply(application_type):
    allowed_types = {
        "first": "First passport",
        "renewal": "Passport renewal",
        "replacement": "Lost or stolen passport replacement",
        "emergency": "Emergency travel document",
    }
    title = allowed_types.get(application_type)
    if title is None:
        abort(404)
    ...
    if request.method == "POST":
        ...
        if not roleplay_name or len(roleplay_name) > 100:
            errors.append("Enter a fictional roleplay name.")
        if not nationality or len(nationality) > 100:
            errors.append("Enter a fictional nationality.")
        if len(reason) < 20 or len(reason) > 1000:
            errors.append("Give a short roleplay explanation between 20 and 1,000 characters.")
        if declaration != "yes":
            errors.append("Confirm this is fictional roleplay information.")
        ...
        if errors:
            for error in errors:
                flash(error, "error")
            return render_template("passport_apply.html", application_type=application_type, title=title)

        now = datetime.utcnow()
        reference = f"PASS-{now.strftime('%Y%m%d')}-{secrets.token_hex(3).upper()}"
        passport_number = f"P{secrets.token_hex(4).upper()}"
        issue_date = now.strftime("%d %B %Y")
        expiry_date = now.replace(year=now.year + 5).strftime("%d %B %Y")
        ...
        db.execute(
            """INSERT INTO passport_applications(
                reference,roblox_username,discord_username,roleplay_name,nationality,
                application_type,reason,passport_number,issue_date,expiry_date,
        ...
        db.commit()
        return redirect(url_for("passport_complete", reference=reference))

    return render_template("passport_apply.html", application_type=application_type, title=title)


@app.route("/passport-office/application/<reference>")
def passport_complete(reference):
    application = get_db().execute(
        "SELECT * FROM passport_applications WHERE reference=?",
        (reference,),
    ).fetchone()
    if application is None:
        abort(404)
    return render_template("passport_complete.html", application=application)



@app.route("/visa/application/<reference>/download.pdf")
def visa_download_pdf(reference):
    a = get_db().execute("SELECT * FROM visa_applications WHERE reference=?", (reference,)).fetchone()
    if a is None: abort(404)
    return _download_pdf(
        f"visa-{a['reference']}.pdf",
        "GOV.MUK Visa Application Confirmation",
        "Save this PDF and keep your application reference.",
        [
            ("Reference", a["reference"]), ("Applicant", a["applicant_name"]),
            ("Roblox username", a["roblox_username"]), ("Discord username", a["discord_username"]),
            ("Visa type", str(a["visa_type"]).title()), ("Destination", a["destination"]),
            ("Arrival date", a["intended_arrival"]), ("Processing time", a["processing_time"]),
            ("Status", str(a["status"]).title()), ("Submitted", a["created_at"]),
        ],
    )


@app.route("/mod/application/<reference>/download.pdf")
def marine_download_pdf(reference):
    a = get_db().execute("SELECT * FROM job_applications WHERE reference=?", (reference,)).fetchone()
    if a is None: abort(404)
    return _download_pdf(
        f"royal-marines-{a['reference']}.pdf",
        "Royal Marines Application Confirmation",
        "Save this PDF. Recruitment staff may request your reference.",
        [
            ("Reference", a["reference"]), ("Unit", a["unit"]),
            ("Roblox username", a["roblox_username"]), ("Discord username", a["discord_username"]),
            ("Age band", a["age_band"]), ("Timezone", a["timezone"]),
            ("Availability", a["availability"]), ("Status", str(a["status"]).title()),
            ("Submitted", a["created_at"]),
        ],
    )


@app.route("/driving-licence/result/<reference>/download.pdf")
def driving_download_pdf(reference):
    a = get_db().execute("SELECT * FROM driving_applications WHERE reference=?", (reference,)).fetchone()
    if a is None or not a["passed"] or not a["licence_number"]: abort(404)
    return _download_pdf(
        f"muk-driving-licence-{a['licence_number']}.pdf",
        "MUK Driving Licence",
        "Download and save this fictional licence PDF.",
        [
            ("Name", a["roleplay_name"]), ("Licence number", a["licence_number"]),
            ("Application reference", a["reference"]), ("Licence type", str(a["licence_type"]).title()),
            ("Theory score", f"{a['score']} / {a['total_questions']}"),
            ("Issue date", a["issue_date"]), ("Expiry date", a["expiry_date"]),
            ("Status", "Valid"),
        ],
    )


@app.route("/passport-office/application/<reference>/download.pdf")
def passport_download_pdf(reference):
    a = get_db().execute("SELECT * FROM passport_applications WHERE reference=?", (reference,)).fetchone()
    if a is None: abort(404)
    return _download_pdf(
        f"muk-passport-{a['passport_number']}.pdf",
        "MUK Roleplay Passport Record",
        "Download and save this fictional passport PDF.",
        [
            ("Name", a["roleplay_name"]), ("Nationality", a["nationality"]),
            ("Passport number", a["passport_number"]), ("Application reference", a["reference"]),
            ("Application type", str(a["application_type"]).replace("_", " ").title()),
            ("Issue date", a["issue_date"]), ("Expiry date", a["expiry_date"]),
            ("Status", str(a["status"]).title()),
        ],
    )


@app.route("/travel/<int:item_id>/download.pdf")
def travel_download_pdf(item_id):
    n = get_db().execute("SELECT * FROM travel_notices WHERE id=?", (item_id,)).fetchone()
    if n is None: abort(404)
    safe = re.sub(r"[^A-Za-z0-9_-]+", "-", n["country"]).strip("-").lower()
    return _download_pdf(
        f"travel-advisory-{safe}.pdf",
        f"{n['country']} Travel Advisory",
        "Fictional GOV.MUK foreign travel advice.",
        [
            ("Country or territory", n["country"]), ("Travel level", str(n["level"]).upper()),
            ("Headline", n["headline"]), ("Advice", n["details"]),
            ("Last updated", n["updated_at"]),
        ],
    )


@app.route("/reference-check", methods=["GET", "POST"])
def reference_check():
    result = None
    result_type = None
    searched_reference = request.values.get("reference", "").strip().upper()

    if searched_reference:
        db = get_db()
        if searched_reference.startswith("VISA-"):
            result = db.execute(
                "SELECT * FROM visa_applications WHERE upper(reference)=?",
                (searched_reference,),
            ).fetchone()
            result_type = "visa"
        elif searched_reference.startswith("RM-"):
            result = db.execute(
                "SELECT * FROM job_applications WHERE upper(reference)=?",
                (searched_reference,),
            ).fetchone()
            result_type = "royal_marines"
        elif searched_reference.startswith("DL-"):
            result = db.execute(
                "SELECT * FROM driving_applications WHERE upper(reference)=?",
                (searched_reference,),
            ).fetchone()
            result_type = "driving"
        elif searched_reference.startswith("PASS-"):
            result = db.execute(
                "SELECT * FROM passport_applications WHERE upper(reference)=?",
                (searched_reference,),
            ).fetchone()
            result_type = "passport"
        else:
            result = db.execute(
                "SELECT * FROM visa_applications WHERE upper(reference)=?",
                (searched_reference,),
    ...
    return render_template(
        ...
        searched_reference=searched_reference,
    )


@app.route("/visa/apply", methods=["GET", "POST"])
def visa_apply():
    if request.method == "POST":
        roblox_username = request.form.get("roblox_username", "").strip()
        discord_username = request.form.get("discord_username", "").strip()
        ...
        intended_arrival = request.form.get("intended_arrival", "").strip()
        intended_stay = request.form.get("intended_stay", "").strip()
        accommodation = request.form.get("accommodation", "").strip()
        declarations = request.form.get("declarations", "").strip()
        ...
        if not applicant_name or len(applicant_name) > 100:
            errors.append("Enter a fictional roleplay applicant name.")
        if not nationality or len(nationality) > 100:
            errors.append("Enter a fictional nationality.")
        if not destination or len(destination) > 100:
            errors.append("Enter a destination.")
        if visa_type not in {"visitor", "work", "diplomatic", "transit"}:
            errors.append("Select a valid visa type.")
        if len(travel_purpose) < 20 or len(travel_purpose) > 1200:
            errors.append("Travel purpose must be between 20 and 1,200 characters.")
        if not intended_arrival:
            errors.append("Enter an intended arrival date.")
        if not intended_stay or len(intended_stay) > 120:
            errors.append("Enter an intended length of stay.")
        if not accommodation or len(accommodation) > 500:
            errors.append("Enter accommodation details.")
        if len(declarations) > 1000:
            errors.append("Declarations must be under 1,000 characters.")
        if confirmation != "yes":
            errors.append("You must confirm this is fictional roleplay information.")

        if errors:
            for error in errors:
                flash(error, "error")
            return render_template("visa_apply.html")

        now = datetime.utcnow().strftime("%Y-%m-%d %H:%M")
        processing_times = {
            "visitor": "10 working days",
            "work": "15 working days",
            "diplomatic": "5 working days",
            "transit": "3 working days",
        }
        processing_time = processing_times.get(visa_type, "10 working days")
        reference = f"VISA-{datetime.utcnow().strftime('%Y%m%d')}-{secrets.token_hex(3).upper()}"
        db = get_db()
        db.execute(
            """INSERT INTO visa_applications(
                reference,roblox_username,discord_username,applicant_name,nationality,
                destination,visa_type,travel_purpose,intended_arrival,intended_stay,
                accommodation,declarations,processing_time,status,created_at,updated_at
            ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (
                reference,
                roblox_username,
        ...
        db.commit()
        return redirect(url_for("visa_complete", reference=reference))

    return render_template("visa_apply.html")


@app.route("/visa/application/<reference>")
def visa_complete(reference):
    application = get_db().execute(
        "SELECT * FROM visa_applications WHERE reference=?",
        (reference,),
    ).fetchone()
    if application is None:
        abort(404)
    return render_template("visa_complete.html", application=application)


@app.post("/admin/visa-application/<int:item_id>/status")
@admin_required
def admin_visa_application_status(item_id):
    status = request.form.get("status", "")
    if status not in {"submitted", "reviewing", "approved", "refused"}:
        flash("Invalid visa application status.", "error")
        return redirect(url_for("admin_dashboard"))

    db = get_db()
    db.execute(
        "UPDATE visa_applications SET status=?,updated_at=? WHERE id=?",
        (status, datetime.utcnow().strftime("%Y-%m-%d %H:%M"), item_id),
    )
    db.commit()
    flash("Visa application status updated.", "success")
    return redirect(url_for("admin_dashboard"))


@app.route("/mod/apply/<unit_slug>", methods=["GET", "POST"])
def royal_marines_apply(unit_slug):
    units = {
        "42-commando": "42 Commando",
        "856-support-unit": "856 Support Unit",
    }
    ...
    if request.method == "POST":
        ...
        criminal_record = request.form.get("criminal_record", "").strip()
        criminal_record_details = request.form.get("criminal_record_details", "").strip()
        ...
        if not timezone or len(timezone) > 80:
            errors.append("Enter your timezone.")
        if len(experience) < 20 or len(experience) > 1500:
            errors.append("Experience must be between 20 and 1,500 characters.")
        if len(motivation) < 30 or len(motivation) > 1500:
            errors.append("Motivation must be between 30 and 1,500 characters.")
        if criminal_record not in {"none", "declared"}:
            errors.append("Select a roleplay criminal-record declaration.")
        if criminal_record == "declared" and not criminal_record_details:
            errors.append("Give a short roleplay explanation of the declared record.")
        if len(criminal_record_details) > 1000:
            errors.append("Roleplay record details must be under 1,000 characters.")
        if not availability or len(availability) > 500:
            errors.append("Enter your usual availability.")
        if declaration != "yes":
            errors.append("You must confirm the application declaration.")

        if errors:
            for error in errors:
                flash(error, "error")
            return render_template("job_application.html", unit=unit, unit_slug=unit_slug)

        now = datetime.utcnow().strftime("%Y-%m-%d %H:%M")
        reference = f"RM-{datetime.utcnow().strftime('%Y%m%d')}-{secrets.token_hex(3).upper()}"
        db = get_db()
        db.execute(
            """INSERT INTO job_applications(
                reference,unit,roblox_username,discord_username,age_band,timezone,
                experience,motivation,criminal_record,criminal_record_details,
        ...
        db.commit()
        return redirect(url_for("job_application_complete", reference=reference))

    return render_template("job_application.html", unit=unit, unit_slug=unit_slug)


@app.route("/mod/application/<reference>")
def job_application_complete(reference):
    application = get_db().execute(
        "SELECT * FROM job_applications WHERE reference=?",
        (reference,),
    ).fetchone()
    if application is None:
        abort(404)
    return render_template(
        "job_application_complete.html",
        application=application,
        discord_url="https://discord.gg/7FAbWyMDfX",
    )


@app.post("/admin/job-application/<int:item_id>/status")
@admin_required
def admin_job_application_status(item_id):
    status = request.form.get("status", "")
    if status not in {"submitted", "reviewing", "accepted", "rejected"}:
...
def admin_dashboard():
    ...
    return render_template("admin.html",
        ...
        job_applications=db.execute("SELECT * FROM job_applications ORDER BY id DESC").fetchall(),
        visa_applications=db.execute("SELECT * FROM visa_applications ORDER BY id DESC").fetchall(),
        stats={
            ...
            "job_applications": db.execute("SELECT COUNT(*) FROM job_applications").fetchone()[0],
            "visa_applications": db.execute("SELECT COUNT(*) FROM visa_applications").fetchone()[0],
        },
...
@app.post("/admin/petition/response/<int:item_id>")
@admin_required
def admin_petition_response(item_id):
    category = request.form.get("category", "Other").strip()
    allowed_categories = {
        "Defence", "Health", "Transport", "Justice", "Foreign Affairs",
        "Economy", "Public Services", "Other"
    ...
    response = request.form.get("government_response", "").strip()
    response_date = datetime.utcnow().strftime("%d %B %Y") if response else ""

    db = get_db()
    db.execute(
        "UPDATE petitions SET category=?,government_response=?,response_date=? WHERE id=?",
        (category, response, response_date, item_id),
    )
    db.commit()
    flash("Petition category and government response updated.", "success")
    return redirect(url_for("admin_dashboard"))
